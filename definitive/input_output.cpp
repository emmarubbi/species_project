...
double spcs::ask_for_output_step() {
    double output_step;
    std::cout << "The simulation will save (and eventually print) the state of the system every x seconds. Please insert x: "; //inserire default
    std::cin >> output_step;
    return output_step;
}
...
bool spcs::print_is_on() {
    bool is_on = false;
    std::cout << "Do you want to print data on terminal? (not advised if you want a lot of data to plot) [y/n]" << std::endl;
    char answer;
    std::cin >> answer;
    ...
    if (!is_on) {
        std::cout << "Note that the data will be displayed after the simulation has ended" << std::endl;
    }
    return is_on;
}
...
void spcs::print(data_set const &data) {
    ...
    std::cout << "Prey number: " << data.preys_ << " Predator number: " << data.predators_ << std::endl;
    std::cout << "Prime integral: " << data.prime_integral_ << std::endl;
}

spcs::data_set::data_set(double time, simulation const &data_to_read) : time_passed_{time}, predators_{data_to_read.get_simulation_state().y_}, 
preys_{data_to_read.get_simulation_state().x_}, prime_integral_{data_to_read.get_prime_integral()} {}

void spcs::update_register(std::vector<data_set> &data_register, data_set const &new_data) {
    data_register.push_back(new_data);
}

void spcs::write_to_file(std::vector<data_set> const &data_register, std::string const &filename) {
    ...
    std::for_each(data_register.begin(), data_register.end(), [&](data_set const &this_data) {
        datafile << this_data.time_passed_ << "\t" << this_data.preys_ << "\t" << 
        this_data.predators_ << "\t" << this_data.prime_integral_ << "\n";
    });
