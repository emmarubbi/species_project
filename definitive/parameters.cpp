...
spcs::pair& spcs::pair::operator/=(const pair &other) {
    if (other.x_ == 0 || other.y_ == 0) {
        throw std::invalid_argument("Error, division by 0"); //WIP, qua viene sollevata solo l'eccezione, che va poi gestita dal resto (funzioni input)
    }
    x_ /= other.x_;
    y_ /= other.y_;
    return *this;
...
bool spcs::sim_parameters::is_invalid_for_simulation() const {
    if (pred_death_ <= 0 || pred_rep_ <= 0 || prey_death_ <= 0 || prey_rep_ <= 0) {
        return true;
    } else {
        return false;
    }
